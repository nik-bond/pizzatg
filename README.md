...
✅ **Готово** — 86 тестов (BDD: команды, платежи, изоляция чатов, энд-ту-энд маршруты)
...
### Недавние изменения
- Изоляция данных по `chat_id`: разные чаты не видят чужие долги
- Интеграционные сценарии бота перенесены в BDD (.feature + steps), legacy-тест удалён
- Миграции SQLite защищены для старых БД (chat_id добавляется до создания индексов)
- Безопасность: `pip-audit` (2.10.0) — уязвимости не найдены
